...
void loop() {
  if (digitalRead(pushButtonP0) == LOW) { // when push button is pressed
      ledcWrite(cServoChannel, degreesToDutyCycle(30)); // set the desired servo position
  } else { // when push button is unpressed
      ledcWrite(cServoChannel, degreesToDutyCycle(150)); // set the desired servo position
  }
